...
const MarqueeItem: React.FC<MarqueeItemProps> = (props) => {
  ...
  //   }
  return (
    <motion.div className="flex flex-row select-none gap-6" ref={itemRef}>
      {children}
    </motion.div>
  );
};
...
export const InteractiveMarquee: React.FC<MarqueeProps> = (props) => {
  ...
  return (
    <>
      {/* <motion.div
        className="bg-caramel-600 fixed top-0 left-0 w-full h-full"
        style={{ opacity }}
        ref={constraintsRef}
      /> */}
      <motion.div
        className="flex items-center cursor-grab z-10 w-full h-full overflow-hidden justify-center gap-6"
        ref={marqueeRef}
        style={{ skewX }}
